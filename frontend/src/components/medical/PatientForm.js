...
const PatientForm = ({ 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        toast.success(`Updated ${result.first_name} ${result.last_name} successfully`);
        ...
        toast.success(`Created ${result.first_name} ${result.last_name} successfully`);
      ...
      toast.error(`${isEditing ? 'Update' : 'Creation'} Failed: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const validateForm = () => {
    ...
    const birthDate = formData.birth_date instanceof Date ? formData.birth_date : new Date(formData.birth_date);
