...
const ModelCreate = () => {
  ...
  const renderFieldInput = field => {
    ...
    // Hide timestamp fields for user creation - they will be auto-generated by backend
    if (modelName === 'user' && (field.name === 'created_at' || field.name === 'updated_at')) {
      return null;
    }

    // Hide active_patient field for user creation - will be auto-generated by backend
    if (modelName === 'user' && field.name === 'active_patient') {
      return null;
    }
          ...
          <div className="form-grid">
            {metadata?.fields
              .filter(field => {
                ...
                if (modelName === 'user') {
                  return !(field.name === 'created_at' || field.name === 'updated_at' || field.name === 'active_patient');
                }
                return true;
              })
              .map(field => {
                const fieldInput = renderFieldInput(field);
                if (!fieldInput) return null;
                
                // Capitalize field labels
                const getFieldLabel = (fieldName) => {
                  if (fieldName === 'email') return 'Email';
                  if (fieldName === 'password_hash' || fieldName === 'password') return 'Password';
                  if (fieldName === 'first_name') return 'First Name';
                  if (fieldName === 'last_name') return 'Last Name';
