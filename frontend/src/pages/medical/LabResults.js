  ...
  const fetchLabResults = useCallback(async () => {
    ...
    try {
      ...
      if (results.length <= 20) {
        ...
        console.log(
          `Skipping file count loading for ${results.length} lab results to prevent rate limiting`
        );
    ...
    try {
      ...
      const labResultData = {
        ...
        ordered_date: formData.ordered_date
          ? new Date(formData.ordered_date).toISOString()
            ...
            try {
              ...
              if (
                ...
                if (
                  ...
                  console.log(
                    `Rate limited for lab result ${result.id}, setting count to 0`
                  );
                } else {
                  console.log(
                    `Error loading files for lab result ${result.id}:`,
                    error
                  );
                }
              }
                ...
                return (
                  <div key={result.id} className="medical-item-card">
                    <div className="medical-item-header">
                      ...
                      <span
                        className={`status-badge status-${result.status.replace(/\s+/g, '-')}`}
                      >
                        {result.status}
                      </span>
                    </div>
                ...
                <div className="details-section">
                  ...
                  <div className="details-grid">
                    ...
                    <div className="detail-item">
                      ...
                      <span
                        className={`status-badge ${getStatusClass(selectedLabResult.status)}`}
                      >
                    ...
                    {selectedLabResult.labs_result && (
                      <div className="detail-item">
                        ...
                        <span
                          className={`result-badge result-${selectedLabResult.labs_result.toLowerCase()}`}
                        >
                          {selectedLabResult.labs_result
                            .charAt(0)
                            .toUpperCase() +
                            selectedLabResult.labs_result.slice(1)}
                        </span>
                      </div>
                    )}
